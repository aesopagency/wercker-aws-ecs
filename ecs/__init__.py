...
class ECSService(object):
    ...
    def register_task_definition(self, family, role_arn, file, volumes):
        """
        ...
        if os.path.isfile(volumes) is False:
            response = self.client.register_task_definition(family=family, taskRoleArn=role_arn, containerDefinitions=container_definitions)
        else:
            with open(volumes, 'r') as content_volumes:
                container_definitions_volumes = json.loads(content_volumes.read())
            response = self.client.register_task_definition(family=family, taskRoleArn=role_arn, containerDefinitions=container_definitions, volumes=container_definitions_volumes)
